...
def get_hidden_weight(weights, user_id):
    if not weights:
        return 0
    return weights.get(user_id, 0) or 0


def violates_weight_rules(team_a, team_b, weights):
    if not weights:
        return False

    for team in (team_a, team_b):
        team_weights = [get_hidden_weight(weights, user_id) for user_id, _role in team]

        # Hard rule: never stack multiple extreme players on the same team.
        if sum(1 for weight in team_weights if weight >= 3) > 1:
            return True
        if sum(1 for weight in team_weights if weight <= -3) > 1:
            return True
...
def score_weight_balance(team_a, team_b, weights):
    if not weights:
        return 0

    weights_a = [get_hidden_weight(weights, user_id) for user_id, _role in team_a]
    weights_b = [get_hidden_weight(weights, user_id) for user_id, _role in team_b]

    score = abs(sum(weights_a) - sum(weights_b)) * 250

    # Soft anti-stacking for strong/weak players that are not extreme enough
    # to trigger the hard rejection above.
    for team_weights in (weights_a, weights_b):
        strong_count = sum(1 for weight in team_weights if weight >= 2)
        weak_count = sum(1 for weight in team_weights if weight <= -2)

        if strong_count > 1:
            score += (strong_count - 1) * 300
        if weak_count > 1:
            score += (weak_count - 1) * 300

    return score


def score_match(players, team_a, team_b, weights=None):
    if violates_weight_rules(team_a, team_b, weights):
        return None
    ...
    weight_penalty = score_weight_balance(team_a, team_b, weights)

    return score_a + score_b + balance_penalty + weight_penalty


def generate_random_teams(players, lobby, weights=None):
    best_result = None
    best_score = None
    ...
    for _ in range(attempts):
        ...
        score = score_match(players, team_a, team_b, weights)
    ...
    # In the unlikely event every sampled split violates a hard weight rule,
    # fall back to a normal role-balanced draft instead of crashing.
    if best_result is None:
        return generate_random_teams(players, lobby, weights=None)

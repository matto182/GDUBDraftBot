...
def score_match(team_a_result, team_b_result):
    preference_total = team_a_result["score"] + team_b_result["score"]
    preference_balance = abs(team_a_result["score"] - team_b_result["score"])

    # Prefer teams with similarly comfortable role assignments after minimizing
    # the total amount of off-primary role usage.
    return preference_total + preference_balance
...
def generate_random_teams(players, lobby):
    ...
    for _ in range(attempts):
        ...
        score = score_match(team_a_result, team_b_result)
